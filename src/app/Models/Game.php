...
class Game extends Model implements PercentageAffiliateCommission
{
    ...
    {
        $gameableType = Str::of($this->gameable_type);

        if((in_array((string) $gameableType, [Slots::class, LuckyWheel::class]) || $gameableType->startsWith('Packages\\GameProviders\\')) && is_object($this->gameable)) {
            return $this->gameable->title;
        }
