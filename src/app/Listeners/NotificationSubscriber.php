...
class NotificationSubscriber
{
    ...
    public function subscribe($events)
    {
        ...
        if (config('settings.users.email_auth') && config('settings.users.email_verification')) {
            $events->listen(Registered::class, [$this, 'sendEmailVerificationLink']);
        }

        
        if (config('settings.notifications.admin.registration.enabled')) {
            $events->listen(Registered::class, [$this,  'sendNewUserAdminNotification']);
        }

        if (config('settings.notifications.admin.game.win.enabled')) {
            $events->listen(GamePlayed::class, [$this, 'sendGameWinAdminNotification']);
        }

        if (config('settings.notifications.admin.game.loss.enabled')) {
            $events->listen(GamePlayed::class, [$this, 'sendGameLossAdminNotification']);
        }
    }
}
