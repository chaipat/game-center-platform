...
class UserController extends Controller
{
    ...
    {
        $user = $request->user();
        $variables = collect($request->only('hide_profit', 'fields'));

        
        if ($request->validated('email') && !$user->hasVerifiedEmail()) {
            $variables->put('email', $request->validated('email'));
        }
    ...
    public function profile(GetUser $request, User $user)
    {
        ...
        return response()->json([
            'user' => $user->only('id', 'name', 'display_name', 'avatar_url', 'created_ago'),
            'stats' => $stats
        ]);
    }
}
