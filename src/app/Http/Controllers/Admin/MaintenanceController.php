...
class MaintenanceController extends Controller
{
    ...
    public function index(CommandManager $commandManager, ReleaseManager $releaseManager)
    {
        ...
        return response()->json([
            ...
            'queues'            => [STAKE_QUEUE_PROVIDER_GAMES, STAKE_QUEUE_MULTIPLAYER_GAMES, 'default'],
            'cron_job_cmd'      => Utils::getCronJobCommand(),
        ]);
    }
    ...
    public function restartQueueWorker()
    {
    ...
    public function getSupervisorServiceStatus()
    {
        if(Utils::platformIsWindows()) {
            return $this->errorResponse(__('This command can not be run on Windows.'));
        }

        foreach (['supervisor', 'supervisord'] as $supervisor) {
            $output = (Process::run("service $supervisor status"))->output();

            if ($output) {
                return $this->successResponse(compact('output'));
            }
        }

        return $this->errorResponse(__('supervisor service is not installed.'));
    }
    ...
    public function getCronJobs()
    {
        ...
        return $error ? $this->errorResponse($error) : $this->successResponse(['output' => $process->output()]);
    }
