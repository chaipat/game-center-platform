...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule): void
    {
        ...
        if ($botsFrequencyMethod) {
            $schedule->command(CreateGames::class)->onOneServer()->$botsFrequencyMethod();
        }

        
        $rainFrequencyMethod = config('settings.bonuses.rain.frequency');
        if ($rainFrequencyMethod) {
            $schedule->command(MakeRain::class)->onOneServer()->$rainFrequencyMethod();
        }

        
        if (config('settings.affiliate.auto_approval_frequency') == 'daily') {
            $schedule->command(ApproveAffiliateCommissions::class)->dailyAt('00:00')->onOneServer();
        } elseif (config('settings.affiliate.auto_approval_frequency') == 'weekly') {
            $schedule->command(ApproveAffiliateCommissions::class)->weeklyOn(1, '00:00')->onOneServer();
        } elseif (config('settings.affiliate.auto_approval_frequency') == 'monthly') {
            $schedule->command(ApproveAffiliateCommissions::class)->monthlyOn(1, '00:00')->onOneServer();
        }

        
        $schedule->command(SeedCommand::class, ['--class' => AssetSeeder::class, '--force'])->daily()->onOneServer();

        collect($this->scheduledCommands)->each(function ($frequency, $command) use ($schedule) {
            $schedule->command($command)->onOneServer()->$frequency();
        });
    }
