            ...
            <v-card :elevation="5" class="recent-game-card" :class="{ win: game.win >= game.bet, loss: game.win < game.bet }">
              ...
              <v-card-text class="body-1 py-0">
                <template v-if="game.win > 0">
                  {{ $t('{0} bet {1} and won {2} credits', [game.account.user.display_name, game.bet, game.win > 999 ? decimal(game.win) : game.win]) }}
                </template>
                <template v-else>
                  {{ $t('{0} bet {1} credits and lost', [game.account.user.display_name, game.bet]) }}
                </template>
              </v-card-text>
