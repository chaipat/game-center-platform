<template>
  <v-list dense class="app-menu-list">
    <template v-for="item in menu">
      <v-list-item
        v-if="item.enabled && canAccess(item.route)"
        :key="item.route"
        :to="{ name: item.route }"
        link
        exact
...
export default {
    ...
    menu() {
      return [
        ...
        },
        {
          ...
          enabled: this.paymentsPackageIsEnabled,
        },
        {
        ...
        {
          route: 'admin.deposit-methods.index',
          icon: 'mdi-cash-marker',
          title: this.$t('Deposit methods'),
          enabled: this.paymentsPackageIsEnabled,
        },
        {
          route: 'admin.withdrawal-methods.index',
          icon: 'mdi-cash-marker',
        ...
        },
        {
          ...
          enabled: this.rafflePackageIsEnabled,
        },
        {
  ...
  methods: {
    canAccess(route) {
      ...
      return (
        this.user.permissions === null || this.user.permissions[module] >= 1
      )
    },
  },
}
