<template>
  <v-card>
    <v-toolbar>
      ...
      <v-toolbar-title class="app-title text-title">
        {{ title }}
      </v-toolbar-title>
      <v-spacer />
      <v-btn v-if="refresh" icon :loading="loading" :disabled="loading" @click="fetchData">
    ...
    <v-card-text>
      ...
      <v-data-table :headers="headers" :items="items" :page="options.page" :sort-by="options.sortBy"
        :sort-desc="options.sortDesc" :items-per-page="options.itemsPerPage" :footer-props="footerProps"
        :server-items-length="itemsTotal" :loading="loading" :must-sort="true" :hide-default-footer="hideFooter"
        :no-data-text="$t('No data found')" :no-results-text="$t('No data found')"
        ...
        <template v-for="(header, slot) of slotHeaderMap" v-slot:[slot]="{ item }">
          ...
          <template v-else>
            {{
              !!header.value
                ? header.format
                  ? format(header.format, get(item, header.value))
...
export default {
  ...
  created() {
    ...
    this.options = this.cacheGet(this.cacheKey) || {
      ...
      search: null,
      filters: this.dataQuery ? this.dataQuery : {},
    }
  },
  ...
  methods: {
    ...
    filterData(filters) {
      ...
      console.log('filters>', this.options.filters)
      this.fetchData()
    },
    searchData(text) {
