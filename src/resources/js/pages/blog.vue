    ...
    <v-container style="max-width: 1185px;">
          ...
          <v-col cols="auto">
            <v-row>
              ...
              <v-col cols="auto" class="pl-0 hide-mobile">
                <span class="text-h6 font-weight-bold"><span class="text-gold-gradient">BLOG</span></span>
              </v-col>
                  ...
                  <v-card class="card2">
                    <v-list-item>
                      <v-list-item-content>
                        ...
                        <v-list-item-subtitle class="text-subtitle-2 my-2 text-style" v-for="item in cat" :key="item.id">
          ...
          <v-row align="center" justify="end">
            <v-col cols="12" sm="auto">
              <v-text-field v-model.trim="searchText" dense filled rounded color="white" clearable hide-details
                ...
                style="background: #24242C;" :class="{ closed: searchBoxClosed && !searchText }"
      ...
      <v-row class="">
        <v-col cols="12" md="8" lg="8">
          ...
          <v-row>
            <v-col>
              <v-pagination class="pagination" v-model="page" :length="total_page" rounded="0" @input="handlePageChange"></v-pagination>
            </v-col>
          </v-row>
        </v-col>
              ...
              <v-card class="card2">
                <v-list-item>
                  <v-list-item-content>
                    ...
                    <hr class="my-3 line__" style="max-width: 10%;">
                    <v-list-item-subtitle v-for="item in newpost" :key="item.id" class="my-2">
                      <v-row>
                        <v-col cols="auto">
                          <!-- <v-img :src=item.cover_image :alt="item.title" ></v-img> -->
                          <a :href="'/blog/' + item.id" v-on:click="ClickItem(item.id)">
                            <v-img style="border-radius: 5px" :src="display_images(item.cover_image)" class="image__blog" :alt="item.title"></v-img>
                          </a>
                        </v-col>
                        <v-col cols="6">
                          <v-row>
...
export default {
  ...
  metaInfo() {
    return {
      title: this.$t("Blog Fair online casino games"),
    };
  },
  computed: {
  ...
  methods: {
    ...
    async GetList(page){
      ...
      await axios.get('/api/content/publish/blog?page=' + page).then((response) => {
    ...
    async GetNewPost(){
      ...
      await axios.get('/api/content/publish/blog').then((response) => {
        ...
        // console.log(response.data.data)
        this.newpost = response.data.data
