                  ...
                  <v-card-text>
                    ...
                    <v-form @submit.prevent="cron">
                      <v-text-field
                        ...
                        :value="data.cron_job_cmd"
                        :disabled="!dataLoaded"
                        @click:append="copyToClipboard($refs.cron)"
                      />
                    </v-form>
                    <v-skeleton-loader type="text" :loading="!dataLoaded">
                      <v-form class="d-inline" @submit.prevent="getCronJobs">
                        <v-btn type="submit" color="primary" :disabled="!dataLoaded || getCronJobsForm.busy" :loading="getCronJobsForm.busy">
                          {{ $t('Cron jobs') }}
                        </v-btn>
                      </v-form>
                      <v-dialog v-model="cronJobsModal">
                        <v-card>
                          <v-toolbar>
                            <v-toolbar-title>
                              {{ $t('Cron jobs') }}
                            ...
                            <v-spacer />
                            <v-btn icon @click="cronJobsModal = false">
                              <v-icon>mdi-close</v-icon>
                            </v-btn>
                          </v-toolbar>
                          <v-card-text>
                            <pre>{{ cronJobsOutput }}</pre>
                          </v-card-text>
                        </v-card>
                      </v-dialog>
                    </v-skeleton-loader>
              ...
              <v-col>
                <v-card outlined>
                  ...
                  <v-card-text>
                    ...
                    <v-row class="mt-2">
                      <v-col cols="12">
                        <v-dialog v-model="queueWorkerModal">
                          <v-card>
                            <v-toolbar>
                              <v-toolbar-title>
                                {{ $t('Supervisor status') }}
                              ...
                              <v-spacer />
                              <v-btn icon @click="queueWorkerModal = false">
                                <v-icon>mdi-close</v-icon>
                              </v-btn>
                            </v-toolbar>
                            <v-card-text>
                              <pre>{{ supervisorStatusOutput }}</pre>
                            </v-card-text>
                          </v-card>
                        </v-dialog>
                        <v-form class="d-inline" @submit.prevent="getSupervisorServiceStatus">
                          <v-btn type="submit" color="primary" :disabled="!dataLoaded || getSupervisorServiceStatusForm.busy" :loading="getSupervisorServiceStatusForm.busy">
                            {{ $t('Supervisor status') }}
                          </v-btn>
                        </v-form>
                        <v-form class="d-inline" @submit.prevent="stopQueueWorker">
                          <v-btn type="submit" color="primary" :disabled="!dataLoaded || restartQueueWorkerForm.busy" :loading="restartQueueWorkerForm.busy">
                            {{ $t('Restart queue worker') }}
                          </v-btn>
                        </v-form>
                      </v-col>
                    </v-row>
...
export default {
  ...
  data () {
    return {
      ...
      cronJobsModal: false,
      queueWorkerModal: false,
      cronJobsOutput: null,
      supervisorStatusOutput: null,
      migrationForm: new Form(),
      cacheForm: new Form(),
      clearQueueForm: new Form({
        queue: null
      }),
      getCronJobsForm: new Form(),
      getSupervisorServiceStatusForm: new Form(),
      restartQueueWorkerForm: new Form()
    }
  },
  ...
  methods: {
    ...
    async stopQueueWorker () {
      const { data } = await this.restartQueueWorkerForm.post(route('admin.maintenance.worker.restart'))

      this.$store.dispatch('message/' + (data.success ? 'success' : 'error'), { text: data.message })
    },
    async getCronJobs () {
      this.cronJobsOutput = null

      const { data } = await this.getCronJobsForm.post(route('admin.maintenance.cron.jobs'))

      if (data.success) {
        this.cronJobsModal = true
        this.cronJobsOutput = data.output
      } else {
        this.$store.dispatch('message/error', { text: data.message })
      }
    },
    async getSupervisorServiceStatus () {
      this.supervisorStatusOutput = null

      const { data } = await this.getSupervisorServiceStatusForm.post(route('admin.maintenance.supervisor.status'))

      if (data.success) {
        this.queueWorkerModal = true
        this.supervisorStatusOutput = data.output
      } else {
        this.$store.dispatch('message/error', { text: data.message })
      }
    },
